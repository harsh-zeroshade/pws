...
export function PageHeroDark({ label, title, titleGold, subtitle, breadcrumb }) {
  return (
    <section className="relative overflow-hidden" style={{ background:"linear-gradient(135deg,#060F1E 0%,#0D2545 60%,#0a1e3d 100%)", minHeight:"52vh", display:"flex", alignItems:"center" }}>
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 lg:px-12 xl:px-16 py-28">
        <motion.nav initial={{ opacity:0,y:10 }} animate={{ opacity:1,y:0 }} transition={{ duration:.5 }}
          className="flex items-center flex-wrap gap-2 mb-8 text-[12px] font-sans">
        ...
        <motion.div initial={{ opacity:0,x:-20 }} animate={{ opacity:1,x:0 }} transition={{ duration:.6 }}
          ...
          <span className="text-[#B8953A] text-[11px] font-semibold tracking-[.3em] uppercase font-sans">{label}</span>
        </motion.div>
        <motion.h1 initial={{ opacity:0,y:40 }} animate={{ opacity:1,y:0 }} transition={{ delay:.1,duration:.9,ease:[.22,1,.36,1] }}
          className="font-display font-black text-white leading-[1.0]" style={{ fontSize:"clamp(2.6rem,6vw,4.8rem)" }}>
        ...
        {subtitle && (
          <motion.p initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} transition={{ delay:.25,duration:.8 }}
            className="text-white/50 text-[16px] leading-relaxed font-sans mt-4 max-w-2xl">{subtitle}</motion.p>
        )}
      </div>
...
/** Member card grid — used by all committee pages */
export function MemberGrid({ members, headLabel = "Head", memberLabel = "Member" }) {
  const ref = useRef(null);
  const inView = useInView(ref, { once:true, margin:"-5%" });
  const heads = members.filter(m => m.role === headLabel || m.isHead);
  const rest  = members.filter(m => m.role !== headLabel && !m.isHead);
  ...
  return (
    <section ref={ref} className="relative overflow-hidden py-20" style={{ background:"linear-gradient(180deg,#060F1E 0%,#0a1628 100%)" }}>
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12 xl:px-16">
        ...
        {/* Head(s) */}
        {heads.length > 0 && (
          <div className={`mb-10 flex flex-wrap gap-5 justify-center ${heads.length === 1 ? "md:justify-start" : ""}`}>
            {heads.map((m, i) => (
              <MemberCard key={i} m={m} i={i} inView={inView} isHead />
            ))}
          </div>
        )}
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {rest.map((m, i) => (
            <MemberCard key={i} m={m} i={i} inView={inView} />
          ))}
        </div>
      </div>
    </section>
...
function MemberCard({ m, i, inView, isHead }) {
  return (
    <motion.div
      ...
      className={`group relative rounded-2xl overflow-hidden flex items-center gap-4 p-5 ${isHead ? "min-w-[280px]" : ""}`}
      style={{
    ...
    >
      {/* Avatar initials */}
      <div className="flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center font-display font-black text-sm"
      ...
      <div className="min-w-0">
        <p className="font-display font-bold text-white text-[14px] leading-tight truncate group-hover:text-[#D4AF5A] transition-colors">{m.name}</p>
        <p className="text-[11px] font-sans mt-0.5 uppercase tracking-wider" style={{ color: isHead ? "#B8953A" : "rgba(255,255,255,0.35)" }}>{m.role}</p>
      </div>
    </motion.div>
  );
}
